import{n,c as S,b as D}from"./header-57de9a26.js";const m=document.querySelector(".mailing_input"),r=document.querySelector(".mailing_form"),_=async e=>{try{const t=r.querySelector(".mailing_button");t.disabled=!0;const s=r.querySelector(".submit-spinner");s.classList.remove("submit-spinner_hide");const o=await fetch("https://your-energy.b.goit.study/api/subscription",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:e})});if(t.disabled=!1,s.classList.add("submit-spinner_hide"),o.ok){const p=await o.json();console.log(`The user has just subscribed with the email: ${e}`),r.reset(),n.Notify.success("We're excited to have you on board! 🎉 Thank you for subscribing to new exercises on Your Energy. You've just taken a significant step towards improving your fitness and well-being.")}else n.Notify.failure("Error occurred while making subscription")}catch(t){const s=r.querySelector(".mailing_button");s.disabled=!1,r.querySelector(".submit-spinner").classList.add("submit-spinner_hide"),console.error(t),n.Notify.failure("Error occurred while making subscription")}};r.addEventListener("submit",e=>{e.preventDefault(),m.checkValidity()?_(m.value):n.Notify.warning("Invalid e-mail address entered.")});const B=100,f=document.querySelector(".scroll-up"),l=document.querySelector(".scroll-up_svg_path"),i=l.getTotalLength(),q=()=>window.pageYoffset||document.documentElement.scrollTop;l.style.strokeDasharray=`${i} ${i}`;l.style.transition="stroke-dashoffset 20 ms";const M=()=>{const e=document.documentElement.scrollHeight-window.innerHeight,t=i-q()*i/e;l.style.strokeDashoffset=t};window.addEventListener("scroll",()=>{M(),q()>B?f.classList.add("scroll-up--active"):f.classList.remove("scroll-up--active")});f.addEventListener("click",()=>{window.scrollTo({top:0,behavior:"smooth"})});const C=document.querySelector(".search-form"),a=document.querySelector(".search-input"),u=document.querySelector(".exercises-back"),k=document.getElementById("icon-search"),E=document.getElementById("icon-x"),v=document.querySelectorAll(".filter-btn"),P=document.querySelector(".filter-category-item"),H="https://your-energy.b.goit.study/api";let I="Body parts",N="Body parts",y=1;const j=12,c={filter:I,category:N,keyword:""};function $(){const{filter:e,category:t,keyword:s}=c;fetch(`${H}/exercises?filter=${e}&category=${t}&keyword=${s}&page=${y}&limit=${j}`).then(o=>o.json()).then(o=>{A()}).catch(o=>(console.error("Sorry, is not found",o),{success:!1}))}v.forEach(e=>{e.addEventListener("click",t=>{v.forEach(s=>s.classList.remove("current")),e.classList.add("current"),c.filter=t.target.dataset.filter,c.category=t.target.dataset.filter,y=1,$()})});C.addEventListener("submit",e=>{e.preventDefault();const t=a.value.toLowerCase();c.keyword=t,y=1,$()});function A(e){u.innerHTML="",e.length===0?u.innerHTML="Sorry, is not found":e.forEach(t=>{const s=O(t);P.addEventListener("click"),u.appendChild(s)})}function O(e){const t=document.createElement("li");return t.classList.add("exCard"),t.dataset.id=e._id,t.innerHTML=` 
    <div class="high-card"> 
    ...
    `,t}a.addEventListener("focus",function(){k.style.display="none",E.style.display="block"});a.addEventListener("blur",function(){k.style.display="block",E.style.display="none",a.value=""});document.querySelector(".filter-category-item");document.querySelector(".filter-section-next");document.querySelector(".filter-section");const d=document.querySelector(".filter-category-list");async function g(e,t=1){try{return(await D.get(`https://your-energy.b.goit.study/api/filters?filter=${e}&page=${t}&limit=12`)).data}catch(s){return console.error("Помилка отримання даних: ",s),null}}function h(e){return d.innerHTML="",e.map(({filter:t,name:s,imgURL:o})=>`<li class="filter-category-item">
              <img
